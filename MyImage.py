...
def imnoise(img, name, *args):
  ...
  out = MyImage(img)
  if name[:2] == 'ga':
    if   len(args) == 2: mu, var = args
    elif len(args) == 1: mu, var = args[0], 0.5*255
    else:                mu, var = 0, 0.5*255
    sigma = var**0.5
    ...
    for y in range(out.getHeight()):
      for x in range(out.getWidth()):
        ...
        g = out[x,y] = out[x,y] + r
        if g < 0.:   out[x,y] = 0.
        if g > 255.: out[x,y] = 255.
        
  elif name[:2] == 'sa':
    if len(args) == 1: dense = args[0]
    else:              dense = 0.05
    denseh = 0.5*dense
    ...
    for y in range(out.getHeight()):
      for x in range(out.getWidth()):
        r = random.random()
        if       r < denseh: out[x,y] = 0
        elif 1.-denseh <= r: out[x,y] = 255
        else:                pass
        
  elif name[:2] == 'sp':
    if len(args) == 1: intense = args[0]
    else:              intense = 0.04
